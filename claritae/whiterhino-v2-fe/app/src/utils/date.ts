...
export const parseSimpleDate = (timestamp: number | string) => {
  ...
  return `${date.getFullYear()}/${formatData(date.getMonth() + 1)}/${formatData(
    date.getDate()
  )} ${formatData(date.getHours())}:${formatData(
    date.getMinutes()
...
export const parseSimpleDateNoSecond = (
  ...
  return `${date.getFullYear()}${connector}${formatData(
    date.getMonth() + 1
  )}${connector}${formatData(date.getDate())} ${formatData(
    date.getHours()
...
export const parseDateNoTime = (
  ...
  return `${year}${formatData(date.getMonth() + 1)}${connector}${formatData(
    date.getDate()
  )}`;
};
...
export const parseDate = (timestamp: number | string) => {
  ...
  if (diff < 3600) {
    return `${Math.floor(diff / 60)}${i18n.t("common.minute")} ${Math.floor(
      diff % 60
    )}${i18n.t("common.second_ago")}`;
  }
...
export const parseTime = (millisecond: number) => {
  ...
  if (hour >= 1) {
    minute %= 60;
    second %= 60;
    return `${Math.floor(hour)} h ${Math.floor(minute)} m ${second.toFixed(
      0
    )} s`;
  }
